...
namespace DigitalSalmon.C360 {
	...
	public class HotspotReactor : BaseBehaviour {
		...
		private readonly HashSet<Hotspot> activeHotspots = new();
		...
		private void ShowHotspots(Node node) {
			...
			foreach (HotspotElement hotspotElement in node.HotspotElements) {
				if (hotspotElement.Hotspot == null) {
					Debug.LogWarning(
						$"Cannot create Hotspot GameObject for {hotspotElement.Name}. Have you assigned a default Hotspot in your Tour? Are you overriding the Hotspot without assigning an override GameObject?");
					continue;
				}
