...
namespace DigitalSalmon.C360 {
	...
	public class PrefabReactor : BaseBehaviour {
		...
		private readonly HashSet<GameObject>    spawnedPrefabs      = new();
		private readonly HashSet<IMappedPrefab> activeMappedPrefabs = new();
		...
		private static void UpdateMappedElementPosition(Transform transform, IMappedElement mappedElement, float distance = -1) {
			...
			transform.localPosition = MathUtility.EquirectangularProjection(new Vector2(mappedElement.Position.x, 1 - mappedElement.Position.y)) *
									(distance == -1 ? MAPPED_ELEMENT_CAMERA_DISTANCE : distance);
